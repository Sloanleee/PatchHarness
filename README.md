...
│  code_review / root_cause_analysis / patch_generation        │
│  test_verify / summary / bug_fix(legacy)                     │
└───────────────────────────┬─────────────────────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────┐
│                    工程能力层                                │
...
说明：项目提供可选真实 LangGraph StateGraph 编排；默认主路径仍使用轻量 sequential workflow，便于本地运行、测试和 benchmark 稳定复现。
...
```text
审查类任务       -> code_review
明确修复任务     -> root_cause_analysis -> patch_generation -> test_verify
完整检查任务     -> code_review -> root_cause_analysis -> patch_generation -> test_verify -> summary
...
动态 Planner：250 次 Agent 调用，估算 500 次 LLM 调用
节省估算 LLM 调用：300 次
节省率：37.5%
规划命中率：100.0%
```
...
│   │   └── langgraph_workflow.py       # 可选真实 LangGraph StateGraph 编排
...
| LangGraph Workflow | `app/graph/langgraph_workflow.py` | 可选真实 StateGraph 节点编排 |
...
| 编排 | Sequential Workflow, optional LangGraph StateGraph |
